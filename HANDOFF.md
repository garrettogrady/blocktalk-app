...
- **Replies have no author join** — `ReplyNode` renders a hardcoded "BlockTalker" username, and the "Replying to @username" chip inherits it. Add a `users(username, user_number)` embed to `ReplyService.fetchReplies` and thread the real username through `ReplyNode` + `onReplyTap`.
...
- **Offline flush is local-only**: going back online moves pending posts into an in-memory `flushed` list shown at the top of the feed — they are NOT actually POSTed to Supabase (session-scoped, gone on relaunch). Wire the real send (create each queued post via `PostService.createPost`, then reload) + swap the debug toggle for `NWPathMonitor`. Offline pin posts currently queue as plain posts (no pin created).
